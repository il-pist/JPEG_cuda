...
NJ_INLINE void njDecodeSOF(void) {
    ...
    for (i = 0, c = nj.comp;  i < nj.ncomp;  ++i, ++c) { // per ogni descrittore di componente (Y/Cb/Cr/R/G/B)
        ...
        if ((c->qtsel = nj.pos[2]) & 0xFC) njThrow(NJ_SYNTAX_ERROR); // pos[2] 2 bit inferiori selezionano quantization table
        njSkip(3);
        nj.qtused |= 1 << c->qtsel;
...
NJ_INLINE void njDecodeScan(void) {
    ...
    njSkip(1);
    for (i = 0, c = nj.comp;  i < nj.ncomp;  ++i, ++c) {
        if (nj.pos[0] != c->cid) njThrow(NJ_SYNTAX_ERROR); // SoScan component item: index must be an increasing integer
